  ...
  checkoutBtn.addEventListener("click", async () => {
    ...
    try {
      const response = await fetch("/api/pay", {
        ...
        body: JSON.stringify({ phone, amount, orderDetails: cartItems, name })
      });

      const data = await response.json();

      if (response.ok && data.ResponseCode === "0") {
        paymentStatus.textContent = "Payment prompt sent! Please complete payment on your phone.";
      } else {
        paymentStatus.textContent = "Payment failed: " + (data.errorMessage || JSON.stringify(data));
      }
    } catch (err) {
